...
public class Character2DMovement : MonoBehaviour
{
	...
	private bool rotating = false;	//added for information about rotating state
	private bool readyJump;			//added for continuous info
	private bool jumping;			//added for continuous animation and no lag after
	...
	private void JumpAndGravity()
	{
		if (mController.isGrounded)
		{
			...
			if (mInput.jump && mJumpTimeoutDelta <= 0.0f && readyJump)
			{
    ...
    void AnimateCharacter()
    {
	    ...
	    if (animator != null)
	    {
			...
			//added for rotation change
			if (mInput.move.x < 0)
			{
				mHeadingRight = false;
				SmoothRotation(Quaternion.Euler(0, -180, 0));
				rotating = true;
			}
			else if (mInput.move.x > 0)
			{
				mHeadingRight = true;
				SmoothRotation(Quaternion.Euler(0, 0, 0));
				rotating = true;
			}

			if (rotating)
			{
				SmoothRotation(mHeadingRight ? Quaternion.Euler(0, 0, 0) : Quaternion.Euler(0, -180, 0));
			}

			void SmoothRotation(Quaternion target)
			{
				transform.localRotation =
					Quaternion.Lerp(transform.localRotation, target, Time.deltaTime * RotationSpeed);
			}
			 ...
			 * These properties can be found in any of the animation controllers
